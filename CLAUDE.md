...
- `questions_de.json` / `questions_en.json` / `questions_fr.json` / `questions_it.json` — the ACTIVE sources (~278 questions each), loaded as `'questions_' + lang + '.json'` by `shared.js` and `index.html`. Each question has `question_number`, `question`, `answers` (object keyed a/b/c/d/e), `correct` (array of keys), and `triagonal_quiz`/`triagonal_pos` — the quiz range and position where the question lives on the triagonal platform (its only user-visible numbering; regenerate by text-matching against a fresh platform harvest if the catalog changes). **`triagonal_quiz` is the platform's own quiz title, not a range of our `question_number`s — never "resync" the two.** They line up for the first four quizzes and then stop: the quiz titled `122-150` holds 36 questions (our q122–q157), because q148–q155 are the eight sub-parts of one matrix question that the platform's older catalog numbering counted as a single item (29 − 1 + 8 = 36). Every other quiz holds exactly its title's span. Rewriting these labels to match `question_number` would invent quiz titles that do not exist on triagonal and break the only way users can locate a question there. Some questions have `linked_to`, `link_order` for multi-part sequences, and `image_url` for diagram references. All four files must stay structurally in sync: same `question_number`s, same answer counts per question. The a/b/c KEY ORDER differs per language by design — do not "align" it. Known exception: q180 has 3 correct answers in IT but 2 in DE/EN/FR because the platform itself disagrees across languages (verified against triagonal 2026-07); the verbatim policy means each file mirrors its own platform language, so do not "fix" this.
